...
All interface references use `{{inputs.NODE_ID.result}}` for referencing. This reference method can be used anywhere input strings are required (including code nodes).
